...
# File paths
MODEL_PATH = os.path.join(MODEL_DIR, "fire_classifier_recovered.json")
...
def load_predictor():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"Recovered model file not found at {MODEL_PATH}"
        )

    # Load recovered native XGBoost Booster
    model = xgb.Booster()
    model.load_model(MODEL_PATH)

    # Scaler remains unchanged
...
def predict_one(features: dict) -> dict:
    ...
    try:
        ...
        # Native XGBoost Booster expects DMatrix
        dmatrix = xgb.DMatrix(scaled_array)

        # Get prediction probabilities
        probabilities = model.predict(dmatrix)[0]

        # For multi:softprob, probabilities are:
        # [class_0_probability, class_1_probability, class_2_probability]
        pred_idx = int(np.argmax(probabilities))

        # Convert internal model index (0, 1, 2)
        # -> original label (0, 2, 3)
        orig_label = reverse_mapping.get(
            pred_idx,
            pred_idx
        )
        ...
        # mapped by original labels
        class_probs = {}

        for idx, prob in enumerate(probabilities):
            label_orig = reverse_mapping.get(
                idx,
                idx
            )

            class_probs[str(label_orig)] = round(
                float(prob),
                4
            )
